...
instance (MonadTransControl t, Monad (t m), MonadReader r m)
    => MonadReader r (Transformed t m) where
  ask = lift Reader.ask
  local fn act = liftWithCarry (\run -> Reader.local fn (run act))
  reader = lift . reader

instance (MonadTransControl t, MonadError e m, Monad (t m))
    => MonadError e (Transformed t m) where
...
instance ( MonadTransControl t, MonadWriter w m, Monad (t m)
         , TransStateFunctor t, TransStatePass t
         ) => MonadWriter w (Transformed t m) where
  writer = lift . writer
...
instance (MonadTrans t, MonadState s m, Monad (t m))
    => MonadState s (Transformed t m) where
  get   = lift State.get
  put   = lift . State.put
  state = lift . State.state

instance (MonadTransControl t, MonadCont m, Monad (t m), TransStateApply t)
    => MonadCont (Transformed t m) where
